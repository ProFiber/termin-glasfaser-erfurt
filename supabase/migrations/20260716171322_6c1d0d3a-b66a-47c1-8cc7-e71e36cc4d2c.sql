...
BEGIN
  INSERT INTO public.contacts (bid,strasse,hnr,hnr_zusatz,plz,ort,name,email,mobil,festnetz,typ,we,ge,zustimmung,lat,lng,auskundung_von,auskundung_bis,nvt,auskundung_erforderlich,auskundung_status,auskundung_erfolgt,auskundung_ergebnis,auftrag_erstellt_am,auftragsquelle,kls_id,fol_id)
  SELECT
    (rec->>'bid')::text,
  ...
  ON CONFLICT (bid) DO UPDATE SET
    ...
    kls_id = coalesce(EXCLUDED.kls_id, contacts.kls_id),
    fol_id = coalesce(EXCLUDED.fol_id, contacts.fol_id);
