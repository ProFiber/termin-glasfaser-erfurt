...
ALTER TABLE public.contacts ADD COLUMN IF NOT EXISTS auftrag_erstellt_am timestamptz;
...
BEGIN
  INSERT INTO public.contacts (bid,strasse,hnr,hnr_zusatz,plz,ort,name,email,mobil,festnetz,typ,we,ge,zustimmung,lat,lng,auskundung_von,auskundung_bis,nvt,auskundung_erforderlich,auskundung_status,auskundung_erfolgt,auskundung_ergebnis,auftrag_erstellt_am)
  SELECT
    (rec->>'bid')::text,
