...
async function importSchmueckeContacts(wb: XLSX.WorkBook, log: Log, dry = false): Promise<{ ok: number; neu: number; upd: number; newAddresses: string[] }> {
  ...
  for (const r of rows) {
    ...
    // Nur unsere freigeschalteten NVT-Gebiete 2V8001–2V8052
    const nvtNum = /^2V(\d{4})$/.exec(nvt)?.[1];
    if (!nvtNum || Number(nvtNum) < 8001 || Number(nvtNum) > 8052) continue;
...
async function importAlleGfStates(wb: XLSX.WorkBook, log: Log, dry = false): Promise<{ ok: number; unmatched: number; dokuIssues: DokuIssue[]; synthAddresses: string[] }> {
  ...
  for (const row of dataRows) {
    ...
    payload.push({
      ...
      grabenlaenge: Math.round(num(row[C.meter])),
      umsatz_eur: num(row[C.umsatz]),
      zusatz_eur: 0,
      foto: foto.val,
      protokoll: proto.val,
      sharepoint: sp.val,
      gf_plus: gfPlus.val,
      eingereicht_am: toDate(row[C.eingereicht]),
      aufmass_am: toDate(row[C.aufmass]),
      gutschrift_nr: str(row[C.gutschrift]),
      avis_am: toDate(row[C.avis]),
      verguetet_am: toDate(row[C.verguetet]),
      bemerkung: str(row[C.bemerkung]),
    });
  }
