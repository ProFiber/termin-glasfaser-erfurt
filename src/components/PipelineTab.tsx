...
type StageKey = "gebaut" | "doku" | "eingereicht" | "aufmass" | "avisiert" | "verguetet";

const STAGES: { key: StageKey; label: string; color: string }[] = [
  { key: "gebaut",      label: "🚧 Gebaut",               color: "#3b82f6" },
  { key: "doku",        label: "📎 Doku",                 color: "#8b5cf6" },
  { key: "eingereicht", label: "📤 Eingereicht",          color: "#f59e0b" },
  { key: "aufmass",     label: "📐 Aufmaß / Gutschrift",  color: "#ec4899" },
  { key: "avisiert",    label: "📬 Avisiert",             color: "#0ea5e9" },
  { key: "verguetet",   label: "💶 Vergütet",             color: "#22c55e" },
];

function hasCompleteDoku(cs: CallState | undefined): boolean {
  if (!cs) return false;
  return !!cs.fotos_erhalten && !!cs.protokoll_erhalten;
}
...
function stageOf(cs: CallState | undefined): StageKey | null {
  ...
  if (hasCompleteDoku(cs)) return "doku";
  return "gebaut";
}
...
function stageDateOf(cs: CallState, stage: StageKey): string | null {
  switch (stage) {
    ...
    case "doku":        return cs.erledigt_datum ?? null;
...
function ageColor(days: number | null, stage: StageKey): string {
  ...
  if (stage === "gebaut" || stage === "doku") {
    if (days > 14) return "#dc2626";
    if (days > 7) return "#f59e0b";
    return "#22c55e";
  }
  // eingereicht / aufmass / avisiert – lange Wartezeit üblich
...
function reasonFor(cs: CallState, stage: StageKey): string | null {
  ...
  if (stage === "gebaut") {
    ...
    return "wartet auf Doku";
  }
  if (stage === "doku") return "wartet auf Einreichung";
  if (stage === "eingereicht") return "wartet auf Aufmaß / Gutschrift per Mail";
  if (stage === "aufmass") return "wartet auf Avis";
...
export default function PipelineTab({ contacts, states }: Props) {
  ...
  const grouped = useMemo(() => {
    const g: Record<StageKey, { c: Contact; cs: CallState; days: number | null }[]> = {
      gebaut: [], doku: [], eingereicht: [], aufmass: [], avisiert: [], verguetet: [],
    };
  ...
  const stuck = grouped.gebaut.filter(x => (x.days ?? 0) > 14).length
             + grouped.doku.filter(x => (x.days ?? 0) > 14).length
             + grouped.eingereicht.filter(x => (x.days ?? 0) > 45).length
             + grouped.aufmass.filter(x => (x.days ?? 0) > 45).length
